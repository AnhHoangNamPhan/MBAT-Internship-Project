...
probe_one <- function(id_str) {
  res <- tryCatch({
    POST(
      ...
      body = list(LNG = lang, CTRISO = country_iso,
                  VEHICLE = "", MODE = "", BRAND = "",
...
probe_range <- function(from_id, to_id) {
  ids <- sprintf("AT.%04d.8", from_id:to_id)
  ...
  n_total <- length(ids)
  if (!n_total) {
    message(sprintf("Chunk %d-%d: nothing to do (all already saved).", from_id, to_id))

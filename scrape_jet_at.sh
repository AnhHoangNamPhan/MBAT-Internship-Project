#!/usr/bin/env bash
# Scrape all Jet stations in Austria

set -euo pipefail

OUT_DIR="data_jet_at"
CACHE_DIR="cache_jet_at"
...
echo "[STEP 1] Scraping station list from Jet Austria"
STATION_LIST_HTML="${CACHE_DIR}/jet_station_list_${TIMESTAMP}.html"
curl -sS "https://www.jet-austria.at/tankstellen-suche" -o "$STATION_LIST_HTML"
...
for ID in $STATION_IDS; do
  ...
  STATION_URL="https://www.jet-austria.at/api/stations/id/${ID}"

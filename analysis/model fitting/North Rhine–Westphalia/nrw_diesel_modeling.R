...
# North Rhine–Westphalia Diesel Fuel Price Modeling (postcode prefixes '4')
...
cat("=== NORTH RHINE–WESTPHALIA DIESEL FUEL PRICE MODELING ===\n")
...
nrw_stations <- dbGetQuery(con_stations, "
  SELECT 
    station_uuid, 
    station_brand,
    latitude,
  ...
  WHERE latitude IS NOT NULL AND longitude IS NOT NULL
    ...
    AND country = 'Germany'
    AND substr(zip_code, 1, 1) IN ('4')
") %>%
  mutate(
    station_uuid = as.character(station_uuid)
  )

cat(sprintf("✓ Loaded %d NRW stations with complete metadata\n", nrow(nrw_stations)))

# =============================================================================
# STEP 2: LOAD DIESEL PRICE DATA FROM NRW (POSTCODE PREFIXES '4')
# =============================================================================
cat("\nSTEP 2: Loading diesel price data from North Rhine–Westphalia (postcode prefixes '4')...\n")

cat(sprintf(" NRW stations (postcode prefixes '4'): %d\n", nrow(nrw_stations)))

# Get prices only for NRW stations using the station list
station_list <- paste0("'", nrw_stations$station_uuid, "'", collapse = ",")
...
unified_data <- german_prices %>%
  left_join(nrw_stations, by = "station_uuid") %>%
...
saveRDS(mdl_1_results, file.path(results_dir, "nrw_diesel_mdl1_metrics.rds"))

cat(sprintf(" Model 1 metrics saved to: %s\n", file.path(results_dir, "nrw_diesel_mdl1_metrics.rds")))
...
saveRDS(mdl_2_results, file.path(results_dir, "nrw_diesel_mdl2_metrics.rds"))

cat(sprintf("    Model 2 metrics saved to: %s\n", file.path(results_dir, "nrw_diesel_mdl2_metrics.rds")))
...
saveRDS(mdl_3_results, file.path(results_dir, "nrw_diesel_mdl3_metrics.rds"))

cat(sprintf("    Model 3 metrics saved to: %s\n", file.path(results_dir, "nrw_diesel_mdl3_metrics.rds")))
...
mdl_1_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl1_metrics.rds"))
mdl_2_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl2_metrics.rds"))
mdl_3_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl3_metrics.rds"))
...
saveRDS(gam_results, file.path(results_dir, "nrw_diesel_gam_metrics.rds"))
cat(sprintf("✓ GAM model results saved to: %s\n", file.path(results_dir, "nrw_diesel_gam_metrics.rds")))
...
for (i in 1:n_smooths) {
  ...
  plot_path <- file.path(results_dir, sprintf("nrw_diesel_gam_term%d_%s.png", i, 
...
xgb_plot_path <- file.path(results_dir, "nrw_diesel_xgb3_learning_curve.png")
...
if (isTRUE(save_model_files)) {
  try(xgb.save(xmdl_3_es, fname = file.path(results_dir, "nrw_diesel_xmdl3.model")), silent = TRUE)
}
...
saveRDS(list(SHAP_MeanAbs = shap_mean_abs, SHAP_Sample = shap_df[1:min(1000, nrow(shap_df)), ]),
        file.path(results_dir, "nrw_diesel_xgb3_shap.rds"))
cat(sprintf("    SHAP summary saved to: %s\n", file.path(results_dir, "nrw_diesel_xgb3_shap.rds")))
...
saveRDS(xmdl_3_results, file.path(results_dir, "nrw_diesel_xmdl3_metrics.rds"))
cat(sprintf(" XGBoost Model 3 metrics saved to: %s\n", file.path(results_dir, "nrw_diesel_xmdl3_metrics.rds")))
...
scale_matrix_in_place <- function(mat, center, scale) {
  for (j in seq_len(ncol(mat))) {
    mat[, j] <- (mat[, j] - center[j]) / scale[j]
  }
  mat
}

train_matrix_nn3 <- scale_matrix_in_place(train_matrix_nn3, scaler_mean_nn3, scaler_sd_nn3)
train_matrix_nn3_scaled <- train_matrix_nn3
rm(train_matrix_nn3); gc(verbose = FALSE)

valid_matrix_nn3 <- scale_matrix_in_place(valid_matrix_nn3, scaler_mean_nn3, scaler_sd_nn3)
valid_matrix_nn3_scaled <- valid_matrix_nn3
rm(valid_matrix_nn3); gc(verbose = FALSE)

test_matrix_nn3 <- scale_matrix_in_place(test_matrix_nn3, scaler_mean_nn3, scaler_sd_nn3)
test_matrix_nn3_scaled <- test_matrix_nn3
...
nn_plot_path <- file.path(results_dir, "nrw_diesel_nn3_learning_curve.png")
...
if (isTRUE(save_model_files)) {
  keras::save_model_hdf5(nn_mdl_3, file.path(results_dir, "nrw_diesel_nn_mdl3_full.h5"))
}
...
saveRDS(nn_mdl_3_results, file.path(results_dir, "nrw_diesel_nn_mdl3_metrics.rds"))

cat(sprintf("    Deep Neural Network Model 3 metrics saved to: %s\n", file.path(results_dir, "nrw_diesel_nn_mdl3_metrics.rds")))
...
mdl1_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl1_metrics.rds"))
mdl2_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl2_metrics.rds"))
mdl3_res <- readRDS(file.path(results_dir, "nrw_diesel_mdl3_metrics.rds"))
gam_res <- readRDS(file.path(results_dir, "nrw_diesel_gam_metrics.rds"))
xgb_res <- readRDS(file.path(results_dir, "nrw_diesel_xmdl3_metrics.rds"))
nn_res <- readRDS(file.path(results_dir, "nrw_diesel_nn_mdl3_metrics.rds"))

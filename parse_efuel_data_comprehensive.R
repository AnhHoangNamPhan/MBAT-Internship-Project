...
extract_charging_points <- function(charging_points_list) {
  if (is.null(charging_points_list) || length(charging_points_list) == 0) {
    return(list(
      total_points = 0,
      charging_point_ids = NA_character_,
  ...
  # Extract charging point IDs
  charging_point_ids <- purrr::map_chr(charging_points_list, ~ (.x$id %or% "UNKNOWN"))
  charging_point_ids_str <- paste(charging_point_ids, collapse = ", ")
  ...
  list(
    ...
    charging_point_ids = charging_point_ids_str,
...
for (file in efuel_files) {
  ...
  df <- purrr::map_dfr(raw$results, function(entry) {
    ...
    tibble(
      ...
      charging_point_ids    = cp_info$charging_point_ids,

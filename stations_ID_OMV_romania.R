...
# -------- config --------
probe_omv_stations <- function(country_iso, lang = "EN", suffix = "8") {
  out_dir <- sprintf("~/Desktop/MBAT-Internship-Project/data_OMV_%s", tolower(country_iso))
  dir.create(path.expand(out_dir), recursive = TRUE, showWarnings = FALSE)
  out_csv <- path.expand(file.path(out_dir, sprintf("stations_%s.csv", country_iso)))
    ...
    res <- tryCatch({
      POST(
        ...
        add_headers(
          ...
          "referer" = "https://www.omv.com/"
        ),
        body = list(
          LNG = lang, CTRISO = country_iso,
          VEHICLE = "", MODE = "", BRAND = "",
  ...
  probe_range <- function(from_id, to_id) {
    ids <- sprintf("%s.%04d.%s", country_iso, from_id:to_id, suffix)
...
# -------- Run for Romania and Serbia --------
probe_omv_stations("RO", lang = "RO")   # Romania
probe_omv_stations("RS", lang = "SR")   # Serbia
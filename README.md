...
- OMV, Avanti, and Diskont are slightly more involved, but work in the same way
...
<details>
<summary>The prices are stored in the PNG</summary>
 - the returned value escapes `\/`
- if we remove that, we can split off the `data:image/png;base64` part into `s` and export in R via `base64decode(clean_base64) |> writeBin("image.png")`
...
We can use OCR to obtain the prices:
```r
library("tesseract")
# OCR the image
